...
public sealed partial class NPCSteeringSystem
{
    ...
    private bool TrySeek(
        ...
        Angle offsetRot,
        float moveSpeed,
        float acceleration, // GREENSHIFT
        float friction, // GREENSHIFT
        Span<float> interest,
        float frameTime,
        ref bool forceSteer, // GREENSHIFT
        ref float moveMultiplier) // GREENSHIFT
    {
        var ourCoordinates = xform.Coordinates;
        var destinationCoordinates = steering.Coordinates;
        var inLos = true;

        // check if we should ignore all pathing logic and go straight to the target coordinates - GREENSHIFT
        ...
        // GREENSHIFT START - EARLY PORT OF https://github.com/space-wizards/space-station-14/pull/38846
        ...
        if (!targetCoordinates.IsValid(EntityManager))
        {
            if (!directMove && steering.CurrentPath.TryPeek(out var poly) && // GREENSHIFT MODIFIED
        ...
        if (arrived)
        {
            ...
            if (!directMove && steering.CurrentPath.TryPeek(out var node) && !IsFreeSpace(uid, steering, node)) // GREENSHIFT MODIFIED
            {
            ...
            if (!directMove && steering.CurrentPath.Count > 0) // GREENSHIFT MODIFIED
            {
                forceSteer = true;
                steering.CurrentPath.Dequeue();
