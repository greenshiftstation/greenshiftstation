...
public sealed partial class NPCSteeringSystem : SharedNPCSteeringSystem
{
    ...
    [Dependency] private readonly GravitySystem _gravity = default!; // GREENSHIFT
        ...
        TimeSpan curTime)
    {
        ...
        var forceSteer = true;
        var moveMultiplier = 1f; // multiplier to acceleration we should actually move with - GREENSHIFT

        if (steering.CanSeek && !TrySeek(uid, mover, steering, body, xform, offsetRot, moveSpeed, acceleration, friction, interest, frameTime, ref forceSteer, ref moveMultiplier)) // GREENSHIFT MODIFIED
        {
            SetDirection(uid, mover, steering, Vector2.Zero);
            return;
        }
